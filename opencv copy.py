...
class simVSexp:
    ...
    def imageProcessing(self):
        ...
        im_gauss = cv2.GaussianBlur(imgray, (3, 3), cv2.BORDER_DEFAULT)
        #test = np.max(imgray)
        # im_gauss = imgray
        # ret, thresh = cv2.threshold(im_gauss, 90, 255, cv2.THRESH_BINARY)
        ret, thresh = cv2.threshold(im_gauss, 80, 255, cv2.THRESH_BINARY)
    ...
    def filterByCalA(self):
        ...
        print(len(self.contours_cirles))
        print('NV for each bubble:')
        for i in range(len(self.contours_cirles)):
            print(len(self.contours_cirles[i]))
        area = 0
        for con in self.contours_cirles:
            area = max(area, cv2.contourArea(con))
        factor = (3.14 * (0.7 ** 2) / area)**(1/2)
        print(factor) 
        for con in self.contours_cirles:
            con = np.array(con) * factor
            for i in con:
                self.debug_file.write(str(i[0][1] - 6) + ',' + str(i[0][0]) + '\n')
